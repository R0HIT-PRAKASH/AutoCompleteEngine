...
class SimplePrefixTree(Autocompleter):
    ...
    def insert(self, value: Any, weight: float, prefix: List) -> None:
        ...
        if prefix is []:
                obj = SimplePrefixTree(self.weight_type)
                obj.value = value
                obj.weight = weight
                self.subtrees.append(obj)
                ...
                self._order_weight()
        elif self._inserted(value, weight, prefix):  # if the value is in tree
            if self.is_leaf() and self.value == value:
                self.weight += weight
            else:
                for subtree in self.subtrees:
                    if subtree._inserted(value, weight, prefix):
...
class CompressedPrefixTree(SimplePrefixTree):
    ...
    def longest_common(self, other: str) -> List[Any]:
        counter = 0
        result = []
        oth = list(other)
